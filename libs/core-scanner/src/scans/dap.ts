...
export const buildDapResult = async (
  ...
  const allGAPropertyIds: string = getAllGAPropertyTags(
    logger,
    outboundRequests,
  );
  ...
  logger.info(
    { dapScriptUrlFromId },
    `Script tag check has returned: ${dapScriptUrlFromId}`,
  );

  const dapScriptCandidateRequests: HTTPRequest[] =
    getDapScriptCandidateRequests(logger, outboundRequests, dapScriptUrlFromId);
  const hasDapScriptCandidateRequests = dapScriptCandidateRequests.length != 0;
  if (!hasDapScriptCandidateRequests) {
    logScanResult(
      logger,
      { hasDapScriptCandidateRequests },
      'dapDetected',
      false,
      'DAP Not Detected',
    );
  ...
  if (!hasDapScriptCandidateRequests && !hasGaPropertyIds) {
    logger.info(
      { hasGaPropertyIds, hasDapScriptCandidateRequests },
      'Unable to locate dap script candidates or GA property IDs.',
    );
    return emptyResponse;
  }
  ...
  if (!hasDapScriptCandidateRequests && hasGaPropertyIds) {
    logger.info(
      { hasGaPropertyIds, hasDapScriptCandidateRequests },
      `No DAP script candidates found, but the following GA property IDs were detected: ${allGAPropertyIds}`,
    );
    return {
      dapDetected: false,
      dapParameters: '',
  ...
  const dapScriptCandidates: DapScriptCandidate[] =
    await getDapScriptCandidates(logger, dapScriptCandidateRequests);

  const dapScript: DapScriptCandidate = getBestCandidate(
    logger,
    dapScriptCandidates,
    dapScriptUrlFromId,
  );
  ...
  if (hasDapScript) {
    ...
    logScanResult(
      ...
      {},
      'dapParameters',
      dapScript.parameters,
      `DAP Parameters found: ${dapScript.parameters}`,
    );
    logScanResult(
      logger,
      {},
      'dapVersion',
      dapScript.version,
      `DAP Version found: ${dapScript.version}`,
    );
  }
 ...
 */
export function getAllGAPropertyTags(
  parentLogger: Logger,
 ...
 */
export function getDapScriptCandidateRequests(
  parentLogger: Logger,
  allRequests: HTTPRequest[],
  dapScriptUrlFromId: string,
): HTTPRequest[] {
  const candidates: HTTPRequest[] = [];
  const logger = parentLogger.child({
    function: 'getDapScriptCandidateRequests',
  });
 ...
 */
export async function getDapScriptCandidates(
  parentLogger: Logger,
  dapScriptCandidateRequests: HTTPRequest[],
): Promise<DapScriptCandidate[]> {
  ...
  for (const request of dapScriptCandidateRequests) {
    ...
    try {
      ...
      logger.info(
        { error: err },
        `Error getting response from candidate body: ${url}`,
      );
      candidate.body = '';
    }

    candidate.version =
      candidate.body !== null ? getDapVersion(candidate.body) : '';
    ...
    try {
      ...
      logger.info(
        { error: err },
        `Error getting post data for DAP script candidate: ${url}`,
      );
    }

    logger.info(
      { dapVersion: candidate.version },
      `DAP script candidate added to list: ${truncatedUrl}`,
    );
...
export function getBestCandidate(
  parentLogger: Logger,
  dapScriptCandidates: DapScriptCandidate[],
  dapScriptUrlFromId: string,
): DapScriptCandidate {
  ...
  const checks = [
    {
      ...
      check: (candidate: DapScriptCandidate, scriptUrl: string) =>
        checkCandidateForScriptAndVersion(candidate) === true,
    },
    {
      name: 'Script By ID and Version',
      check: (candidate: DapScriptCandidate, scriptUrl: string) =>
        checkCandidateForScriptUrlFromIdAndVersion(candidate, scriptUrl) ===
        true,
    },
    {
      name: 'Property and Version',
      check: (candidate: DapScriptCandidate, scriptUrl: string) =>
        checkCandidateForPropertyAndVersion(candidate) === true,
    },
    {
      name: 'Version',
      check: (candidate: DapScriptCandidate, scriptUrl: string) =>
    ...
    },
    {
      ...
      check: (candidate: DapScriptCandidate, scriptUrl: string) =>
        checkUrlForScriptNameMatch(candidate.url) === true,
    },
    {
      name: 'Any DAP Match',
      check: (candidate: DapScriptCandidate, scriptUrl: string) =>
        checkCandidateForAnyDapMatch(candidate) === true,
    },
  ];
  ...
  for (const candidate of dapScriptCandidates) {
    ...
    if (matchLevel < bestMatchLevel) {
      logger.debug(
        `DAP script candidate is the best match so far: ${truncatedUrl}`,
      );
 ...
 */
export function checkCandidateForScriptAndVersion(
 ...
 * @returns TRUE if the DapScriptCandidate contains the exact dap script URL and a version, FALSE otherwise
 */
export function checkCandidateForScriptUrlFromIdAndVersion(
  candidate: DapScriptCandidate,
  scriptUrl: string,
): boolean {
 ...
 */
export async function checkPageForScriptTag(
  logger: Logger,
  page: Page,
): Promise<boolean> {
 ...
 */
export async function getUrlByScriptId(
  logger: Logger,
  page: Page,
): Promise<string | null> {
 ...
 */
export function checkCandidateForPropertyAndVersion(
  candidate: DapScriptCandidate,
): boolean {
 ...
 */
export function checkCandidateForAnyDapMatch(
  candidate: DapScriptCandidate,
): boolean {
  ...
  const isPostDataMatch = checkPostDataForPropertyIdMatch(candidate.postData);
  return (
    !!isPropertyIdMatch || isPostDataMatch || isExactScriptMatch || hasVersion
  );
}
