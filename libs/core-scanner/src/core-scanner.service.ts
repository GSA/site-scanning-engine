...
import {
  ...
  PageScanFailure,
  PageScanSuccess,
} from 'entities/scan-page.entity';
...
@Injectable()
export class CoreScannerService implements Scanner<
  CoreInputDto,
  CoreResultPages
> {
  constructor(
  ...
  async scan(input: CoreInputDto): Promise<CoreResultPages> {
    ...
    if (input.page) {
      scanLogger.warn(
        `Page filtering enabled! Only '${input.page}' will be executed. This should never be used in production!`,
      );
    }

    if (input.scan) {
      scanLogger.warn(
        `Scan filtering enabled! Only '${input.scan}' will be executed. This should never be used in production!`,
      );
    }
    ...
    return await this.browserService.useBrowser(async (browser) => {
      ...
      for (const page in Page) {
        ...
        if (Object.prototype.hasOwnProperty.call(Page, page)) {
          ...
          result[pageName] = await this.runPage(
            pageName,
            input,
            scanLogger,
            browser,
  ...
  private shouldRunPage(
    ...
    if (page.toLowerCase().includes(input.page.toLowerCase())) {
      pageLogger.info(
        `Page '${page}' includes page filter '${input.page}'; running page.`,
      );
      return true;
    }
    pageLogger.warn(
  ...
  private async runPage(
    ...
    try {
      const pageResult = await this.getPageResult(
        page,
        input,
        pageLogger,
        browser,
  ...
  private logPageSuccess(
    timer: DurationLogTimer,
    page: Page,
    input: CoreInputDto,
    pageLogger: Logger,
  ): void {
    timer.log(
      {},
      `scanner.page.${page}.overall.duration.total`,
      `Page '${page}' completed successfully for site '${input.url}' in [{metricValue}ms]`,
    );
    logCount(
      pageLogger,
      {},
      `scanner.page.${page}.succeeded.count`,
      `Counting successful page '${page}' completion for site '${input.url}'.`,
    );
  }

  private logPageFailure(
    timer: DurationLogTimer,
    page: Page,
    input: CoreInputDto,
    pageLogger: Logger,
    ...
    timer.log(
      {},
      `scanner.page.${page}.overall.duration.total`,
      `Page '${page}' failed for site '${input.url}' after [{metricValue}ms]`,
    );
    logCount(
      pageLogger,
      {},
      `scanner.page.${page}.failed.count`,
      `Counting page '${page}' failure for site '${input.url}'.`,
    );
    pageLogger.error(
      { error },
      `Error running '${page}' page: ${error.message}`,
    );
  }

  private logCoreScannerCompletion(
    timer: DurationLogTimer,
    input: CoreInputDto,
    scanLogger: Logger,
  ): void {
    timer.log(
      {},
      'scanner.core.site.duration.total',
      `Scan completed for site '${input.url}' in [{metricValue}ms]`,
    );
    logCount(
      scanLogger,
      {},
      'scanner.core.site.scanned.count',
      `Counting successful scan completion for site '${input.url}'.`,
    );
  }

  private async getPageResult(
    page: Page,
    input: CoreInputDto,
    pageLogger: Logger,
    browser: Browser,
    ...
    switch (page) {
      ...
      case Page.ROBOTS_TXT:
        return this.browserService.processPage(
          browser,
          pages.createRobotsTxtScanner(pageLogger, input),
        );

      case Page.SITEMAP_XML:
        return this.browserService.processPage(
          browser,
          pages.createSitemapXmlScanner(pageLogger, input, this.httpService),
        );
      ...
      case Page.ACCESSIBILITY:
        return {
          accessibilityScan: await this.browserService.processPage(
            browser,
            pages.createAccessibilityScanner(pageLogger, input),
          ),
        };
      ...
      case Page.WWW:
        return await this.browserService.processPage(
          browser,
          pages.createWwwScanner(pageLogger, input),
        );
    }
  }

  private getScanStatus(
    error: Error,
    url: string,
    logger: Logger,
  ): AnyFailureStatus {
    ...
    logCount(
      logger,
      {},
      `scanner.core.status.error.${scanStatus}.count`,
      `Counting failed Scan Status ('${scanStatus}') for site '${url}'.`,
    );
    return scanStatus;
  }
}
